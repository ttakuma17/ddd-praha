...
import com.ddd.praha.infrastructure.persistence.typehandler.TaskStatusTypeHandler;
...
public interface MemberTaskMapper {
    ...
    @Results({
        @Result(property = "owner", column = "member_id", javaType = Member.class, 
                one = @One(select = "com.ddd.praha.infrastructure.persistence.mapper.MemberMapper.findById")),
     ...
     * @return 課題とステータスのマップ
     */
    @Select("SELECT t.id as task_id, t.name as task_name, mt.status " +
            "FROM tasks t " +
            "LEFT JOIN member_tasks mt ON t.id = mt.task_id AND mt.member_id = #{memberId}")
    @Results({
        @Result(property = "key", column = "task_id", javaType = Task.class, 
                one = @One(select = "com.ddd.praha.infrastructure.persistence.mapper.TaskMapper.findById")),
        @Result(property = "value", column = "status", javaType = TaskStatus.class, 
                typeHandler = TaskStatusTypeHandler.class)
    })
    Map<Task, TaskStatus> findTaskStatusMapByMemberId(@Param("memberId") String memberId);
